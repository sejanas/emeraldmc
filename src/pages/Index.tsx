...
import { healthPackages, tests, testCategories } from "@/data/siteData";
...
const doctors = [
  { name: "Dr. Ashok Menon", spec: "MD Pathology", img: drAshok },
  { name: "Dr. Priya Sharma", spec: "MD Biochemistry", img: drPriya },
  { name: "Dr. Rakesh Kumar", spec: "MD General Medicine", img: drRakesh },
];
    ...
    <section className="relative overflow-hidden bg-section-gradient">
      <div className="container grid items-center gap-8 py-16 md:grid-cols-2 md:py-24">
        <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={0}>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            <Button asChild variant="outline" size="lg">
              <Link to="/packages">View Health Packages</Link>
            </Button>
          </div>
        </motion.div>
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.2 }}>
          <img
            src={heroImg}
            alt="Emerald Medical Care - ISO certified diagnostic laboratory in Sri Vijaya Puram"
            className="w-full rounded-2xl card-shadow object-cover"
            loading="eager"
          />
        </motion.div>
      </div>
      ...
      <div className="container">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {tests.slice(0, 6).map((t, i) => (
            ...
            >
              <div>
                <p className="text-xs font-medium text-primary">{t.category}</p>
                <h3 className="mt-1 font-semibold text-foreground">{t.name}</h3>
                <p className="mt-1 text-xs text-muted-foreground">{t.reportTime} • {t.sampleType}</p>
              </div>
        ...
        <div className="mt-8 text-center">
          <Button asChild variant="outline">
            <Link to="/tests">View All Tests <ArrowRight className="ml-2 h-4 w-4" /></Link>
          </Button>
        </div>
      </div>
    </section>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {healthPackages.map((pkg, i) => (
          <motion.div
            ...
            variants={fadeUp}
            custom={i}
            className={`relative rounded-xl border p-6 transition-shadow hover:card-shadow-hover ${
          ...
          >
            ...
            <Button asChild className="mt-5 w-full" size="sm" variant={pkg.popular ? "default" : "outline"}>
    ...
    <section className="container py-16">
      <div className="rounded-2xl bg-primary p-10 text-center md:p-16">
        ...
        <p className="mx-auto mt-3 max-w-md text-primary-foreground/80">
          Schedule your appointment today and get accurate diagnostic results from our ISO certified lab.
        </p>
