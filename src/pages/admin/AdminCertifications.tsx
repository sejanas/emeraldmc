...
const emptyForm = { name: "", slug: "", issuing_authority: "", description: "", image_url: "", display_order: 0, is_active: true };
...
const AdminCertifications = () => {
  ...
  const openNew = () => { setEditing(null); setForm({ ...emptyForm, display_order: (certsQuery.data ?? []).length }); setOpen(true); };
  const openEdit = (c: any) => {
    setEditing(c);
    setForm({ name: c.name, slug: c.slug, issuing_authority: c.issuing_authority || "", description: c.description || "", image_url: c.image_url || "", display_order: c.display_order, is_active: c.is_active });
    setOpen(true);
  };
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        await updateCert.mutateAsync({ id: editing.id, ...form });
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div><Label>Issuing Authority</Label><Input value={form.issuing_authority} onChange={(e) => setForm({ ...form, issuing_authority: e.target.value })} className="mt-1" /></div>
